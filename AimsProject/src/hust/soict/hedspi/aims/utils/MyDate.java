...
public class MyDate {
    ...
    public MyDate(String day, String month, String year) {
        ...
        String unitsArray[] = { "zero", "one", "two", "three", "four", "five", "six", 
                      "seven", "eight", "nine", "no", "eleven", "twelve",
	...
	String tensArray[] = { "zero", "ten", "twenty", "thirty", "forty", "fifty",
        ...
        for(int i=0;i<yearList.length;i++) {
        	for(int j=0;j<unitsArray.length;j++) {
        		if(yearList[i].equals(unitsArray[j])) {
        			check1 = true;
        			break;
        		}
        	}
        	for(int j=0;j<tensArray.length;j++) {
        		if(yearList[i].equals(tensArray[j])) {
        			check2 = true;
        			break;
        		}
        	}
        ...
        switch (yearList.length) {
            ...
            case 2:
                ...
                break;
            default:
            	System.out.println("Inavalid year");
                break;
        }

        temp_year = yNumber;
        Calendar fDate = Calendar.getInstance();
        fDate.set(temp_year, temp_month, 1);
        if(temp_day < 1 || temp_day > fDate.getActualMaximum(Calendar.DAY_OF_MONTH) ){ //getActualMaximum to get the last day of month
            System.out.println("Not valid date.");
            return;
        }
        
        this.iDay = temp_day;
        this.iMonth = temp_month;
        this.iYear = temp_year;
    }
    ...
    public void accept() {
        ...
        formatter.setLenient(false);
        try {
            bDate = formatter.parse(strDate.trim());
        } catch (ParseException pe) {
